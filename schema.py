...
def init_tables():
    ...
    # --- 1. TABLES ---
    session.sql("CREATE TABLE IF NOT EXISTS VITALIS_DEV.APP.CUSTOMER_SOURCE (SOURCE_ID VARCHAR PRIMARY KEY, SOURCE_NAME VARCHAR, IS_ACTIVE BOOLEAN DEFAULT TRUE)").collect()
    ...
    # SP: Add Customer
    session.sql("""
        CREATE PROCEDURE IF NOT EXISTS VITALIS_DEV.APP.SP_ADD_CUSTOMER(
    ...
    session.sql("""
        CREATE PROCEDURE IF NOT EXISTS VITALIS_DEV.APP.SP_UPDATE_CUSTOMER(
            p_id VARCHAR, p_name VARCHAR, p_email VARCHAR, p_phone VARCHAR,
    ...
    # SP: Update Notes
    session.sql("""
        CREATE PROCEDURE IF NOT EXISTS VITALIS_DEV.APP.SP_UPDATE_NOTES(p_id VARCHAR, p_notes VARCHAR, p_user VARCHAR)
        RETURNS VARCHAR LANGUAGE SQL AS $$
        BEGIN
    ...
    session.sql("""
        CREATE PROCEDURE IF NOT EXISTS VITALIS_DEV.APP.SP_DELETE_CUSTOMER(p_id VARCHAR, p_user VARCHAR)
        RETURNS VARCHAR LANGUAGE SQL AS $$
        BEGIN
    ...
    # SP: Add Report
    session.sql("""
        CREATE PROCEDURE IF NOT EXISTS VITALIS_DEV.APP.SP_ADD_REPORT(p_report_id VARCHAR, p_customer_id VARCHAR, p_filename VARCHAR, p_stage_path VARCHAR, p_user VARCHAR)
        RETURNS VARCHAR LANGUAGE SQL AS $$
        BEGIN
    ...
    # SP: Delete Report
    session.sql("""
        CREATE PROCEDURE IF NOT EXISTS VITALIS_DEV.APP.SP_DELETE_REPORT(p_report_id VARCHAR, p_customer_id VARCHAR, p_user VARCHAR)
        RETURNS VARCHAR LANGUAGE SQL AS $$
        BEGIN
    ...
    # SP: Extract Report
    session.sql("""
         CREATE PROCEDURE IF NOT EXISTS VITALIS_DEV.APP.SP_EXTRACT_REPORT(
             P_REPORT_ID VARCHAR, 
